...
# Figure 7a - Transcription factor scatter plot
...
# Figure 7b - Heatmap of CRE accessibility dynamics in pseudotime
multiome_pt <- subset(multiome, subset=Annotation.Lvl1=='PT')
...
# Heatmap
avg_expr_1 <- avg_expr[rownames(avg_expr)%in%regions[regions$cluster==c('early'), 'regions'],]
avg_expr_2 <- avg_expr[rownames(avg_expr)%in%regions[regions$cluster==c('mid'), 'regions'],]
avg_expr_3 <- avg_expr[rownames(avg_expr)%in%regions[regions$cluster==c('late'), 'regions'],]
pheatmap(avg_expr_3, cluster_cols=F, scale='row',
         show_rownames=F, 
         ...
         color=c(viridis(1200, option='E', begin=0, end=0.2),
                 viridis(400, option='E', begin=0.2, end=0.8),
                 viridis(1000, option='E', begin=0.8, end=1)), 
...
# Figure 7c - Histogram of TF binding sites in CREs
regulons <- read.csv(file.path(path, 'regulons_pt_expressed.csv'))
regions <- read.csv(file.path(path, 'pt_pseudotime_regions.csv'))
...
ggplot(data=n_binding, aes(x=reorder(TF, N), y=N, fill=source)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c('grey50', '#440154', '#35b779', '#fde725')) +
  theme_bw() +
  ...
  theme(axis.title.y = element_text( size=12, margin = margin(r = 15)),
        ...
        legend.text = element_text(size=12, color='black')) + theme(legend.position = 'right') + 
  coord_flip() + NoLegend()
...
# Figure 7d - UMAP plot of AP1/NFKB scores
multiome_pt <- subset(multiome, subset=Annotation.Lvl1=='PT')
regulons <- read.csv(file.path(path, 'regulons_pt_expressed.csv'))
# Score AP1
...
# Figure 7e - Footprinting of AP1/NFKB
multiome_pt <- subset(multiome, subset=Annotation.Lvl1=='PT')
...
# Figure 7f - Motif enrichment in CUT&RUN overlap peaks
multiome_pt <- subset(multiome, subset=Annotation.Lvl1=='PT')
...
DefaultAssay(multiome_pt) <- 'ATAC'
da_jun <- RunPresto(multiome_pt, ident.1 = c('PT Inflammatory'), ident.2 = c('PT S1', 'PT S2', 'PT S3'), 
...
ggplot(da_jun, aes(x=avg_log2FC, y=-log10(p_val), color=col)) +
  ...
  labs(color = "") + 
  theme(axis.title.y = element_text(size=14, margin = margin(r = 15)),
        axis.text.x = element_text(size=12, angle = 0, hjust = 0.5, color = "grey10"),
        axis.title.x = element_text(size=14, color = "black"),
        axis.text.y = element_text(size=12, color = "black"),
        legend.title = element_text(size=14, color="black"),
...
da_nfkb <- RunPresto(multiome_pt, ident.1 = c('PT Inflammatory'), ident.2 = c('PT S1', 'PT S2', 'PT S3'), 
                     logfc.threshold = 0, min.pct = 0.00)
...
ggplot(da_nfkb, aes(x=avg_log2FC, y=-log10(p_val), color=col)) +
  ...
  theme(axis.title.y = element_text(size=14, margin = margin(r = 15)),
        axis.text.x = element_text(size=12, angle = 0, hjust = 0.5, color = "grey10"),
        axis.title.x = element_text(size=14, color = "grey10"),
...
enriched.motifs %>%
  ...
  geom_bar(stat = "identity", fill = "#800080", color='black', alpha = 0.7, width = 0.8) +
...
enriched.motifs.nfkb %>%
  arrange(desc(-fold.enrichment)) %>%
  ggplot(aes(x = factor(motif.name, levels = motif.name), y = fold.enrichment)) +
  geom_bar(stat = "identity", fill = "#4d99ca", color='black', alpha = 0.7, width = 0.8) +
  coord_flip() + ylim(c(0, 7)) +
  theme_classic() +
  theme(axis.title.y = element_text(size=14, margin = margin(r = 15)),
        axis.text.x = element_text(size=12, angle = 0, hjust = 0.5, color = "black"),
...
# Figure 7g - Genome plot of target genes
multiome_pt <- subset(multiome, subset=Annotation.Lvl1=='PT')
multiome_pt$class <- multiome_pt$Annotation.Lvl2
...
# Figure 7h - Network plot generated from SCENIC+ analysis
